...
NSA_TOPK_SELECTION_POLICY="${NSA_TOPK_SELECTION_POLICY:-${SPARKINFER_NSA_TOPK_SELECTION_POLICY:-bounded_compat}}"
...
case "${NSA_TOPK_SELECTION_POLICY}" in
  exact|bounded_compat) ;;
  *) die "NSA_TOPK_SELECTION_POLICY must be exact or bounded_compat" ;;
esac
...
export SPARKINFER_NSA_TOPK_SELECTION_POLICY="${NSA_TOPK_SELECTION_POLICY}"
...
if [[ "${DRY_RUN:-0}" == "1" ]]; then
  ...
  printf 'SPARKINFER_NSA_TOPK_SELECTION_POLICY=%q\n' \
    "${SPARKINFER_NSA_TOPK_SELECTION_POLICY}"

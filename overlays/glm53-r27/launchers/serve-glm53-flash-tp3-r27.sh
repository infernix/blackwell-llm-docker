...
# Strict GLM-5.3-Flash TP3 runtime policy (R27 launcher chain).
#
# This launcher mirrors the semantic intent of the TP3 runtime work done on
# R21 and older bases but is built on the R27 cache-complete chain. It is
# fail-closed: every value that shapes the TP3 runtime is locked, and caller
# overrides are rejected before startup.
...
case "${TP:-}" in
  3) ;;
  *) fail "R27 TP3 policy requires TP=3; got ${TP:-unset}" ;;
esac
lock_env() {
  local name=$1 expected=$2
  if [[ -v ${name} && ${!name} != "${expected}" ]]; then
    fail "R27 TP3 ${name} is locked to ${expected}; got ${!name}"
  fi
...
require_unset_env() {
  local name=$1
  if [[ -v ${name} ]]; then
    fail "R27 TP3 ${name} must be unset; got ${!name}"
  fi
}

# CACHE_MODE=vram keeps the qualified dense envelope (default).
# CACHE_MODE=lmcache admits the LMCache DRAM tier with the same locked TP3
# geometry. CACHE_MODE=native admits the host DRAM offload path via the R27
# parent's native KV cache connector with the same locked TP3 geometry.
...
case ${CACHE_MODE:-vram} in
  vram)
    if [[ ${LMCACHE_ENABLED:-0} != 0 ]]; then
      fail 'R27 TP3 dense-cache policy does not support LMCache'
    fi
    ;;
  lmcache)
...
if [[ ${DCP:-1} != 1 ]]; then
  fail "R27 TP3 policy requires DCP=1; got ${DCP:-unset}"
fi
if [[ ${MM_ENCODER_TP_MODE:-weights} != weights ]]; then
  fail "R27 TP3 requires MM_ENCODER_TP_MODE=weights; got ${MM_ENCODER_TP_MODE:-unset}"
fi
if (($# > 0)) && [[ $1 != -* ]]; then
  fail 'R27 TP3 target model is locked; positional model overrides are forbidden'
fi
if [[ -v VLLM_GLM53_SPLIT_TARGET_BLOCK_SIZE ||
      -v VLLM_GLM53_SPLIT_MAMBA_BLOCK_SIZE ]]; then
  fail 'R27 TP3 dense cache rejects split-page block-size variables'
fi
...
case "${tp3_speculator}" in
  mtp)
    ...
    case "${tp3_speculative_tokens}" in
      0 | 3) ;;
      *) fail "R27 TP3 MTP depth must be 0 or 3; got ${tp3_speculative_tokens}" ;;
    esac
    ;;
  dflash | dflash2)
    ...
    [[ ${tp3_speculative_tokens} == 7 ]] ||
      fail "R27 TP3 DFlash2 depth must be 7; got ${tp3_speculative_tokens}"
    ;;
  *) fail "R27 TP3 SPECULATOR must be mtp or dflash2; got ${tp3_speculator}" ;;
esac
...
if (($# > 0)); then
  fail "R27 TP3 policy rejects caller option ${1%%=*}"
fi
...
lock_env VLLM_MTP_NVFP4_LM_HEAD 1
...
case "${GPU_MEMORY_UTILIZATION}" in
  0.91) ;;
  *) fail "R27 TP3 GPU_MEMORY_UTILIZATION is locked to 0.91; got ${GPU_MEMORY_UTILIZATION}" ;;
esac
...
readonly fingerprint=cu133-torch213-glm53-r27-tp3-vllmb033572b-b12xf52b0824-dense-ctx1m-seq8-bt8192

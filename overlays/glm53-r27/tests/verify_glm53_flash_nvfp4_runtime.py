#!/usr/bin/env python3
# Extended verifier: R27 NVFP4 runtime + strict TP3 policy checks, --policy-only mode.
...
TP3_LAUNCHER = pathlib.Path("/usr/local/bin/serve-glm53-flash-tp3-r27.sh")
...
LOCKED_FINGERPRINT = (
    "cu133-torch213-glm53-r27-tp3-vllmb033572b-b12xfdb5bc7b-dense-ctx1m-seq8-bt8192"
)
GENERIC_FINGERPRINT = (
...
EXPECTED_SOURCE_LOCK_SHA256 = (
    "c94ce47bc4101d2256f597675481ad8f7ed2e7a52852f107d7bd0882b0611a34"
)
EXPECTED_SOURCE_LOCK_FIELDS = {
    ...
    "lmcache.commit": "822330db1160d2847538d53f9e6f4eeee2404f4d",
    "lmcache.tree": "008ac3e09ae5917aa0849147480d7bd5b9f8b37a",
    "lmcache.package.tree": "fe5442fbf258accaa7f26d2bbb00d8b7b5c349ca",
...
EXPECTED_DELEGATE_SHA256 = {
    EXPECTED_DFLASH_DELEGATE: (
        "e149d469b999181ca626c450c12f531721f21c0fb697690f80c7617f591ea7b4"
    ),
    EXPECTED_GENERIC_DELEGATE: (
        "a618a5c6e60e0b2d145cbf6a269cddb7fb032abd45d5b6ddc8d769150da15d02"
    ),
}
LOCKED_ENV = (
    ("TP", "3"),
...
def verify_tp3_launcher() -> None:
    ...
    assert 'exec /usr/local/bin/serve-glm53-flash-tp3-r27.sh "$@"' in dispatcher_text
    ...
    assert "VLLM_MTP_NVFP4_LM_HEAD" in tp3_text
...
def run_tp3_policy_cases() -> None:
    ...
    try:
        ...
        dispatcher_text = substitute(
            ...
            "/usr/local/bin/serve-glm53-flash-tp3-r27.sh",
            str(sandbox_tp3),
        )
        dispatcher_text = substitute(
        ...
        # Inherited parent defaults are translated to TP3 values rather than
        ...
        # geometry; native is now accepted on the R27 chain.
        result = subprocess.run(
            ["bash", str(sandbox_dispatcher)],

...
grep -Fxq \
  ...
  <<<"${output_r31}"
grep -Fxq \
  'launcher_ref=fe8f720ac2f9434ef25b0f617fc8a08ca2c0bd1a' \
  <<<"${output_r31}"
grep -Fxq \
  'launcher_commit=fe8f720ac2f9434ef25b0f617fc8a08ca2c0bd1a' \
  <<<"${output_r31}"
grep -Fxq \
...
grep -Fxq 'composition=reproduce-r29' <<<"${output_r29}"
grep -Fxq \
  'image=voipmonitor/vllm:gilded-gnosis-v20-vllm55db472-b12x6bc35fd-fi801d57a-cu132-20260807-r29' \
  <<<"${output_r29}"
grep -Fxq \
  'version=0.11.2.dev280+gilded.gnosis.v20.vllm55db472.b12x6bc35fd.fi801d57a.cu132.20260807.r29' \
  <<<"${output_r29}"
grep -Fxq \
...
grep -Fxq \
  ...
  <<<"${output_r29}"
grep -Fxq \
  'lmcache_patch=releases/gilded-gnosis-v20-r29/lmcache/integration.patch' \
  <<<"${output_r29}"
grep -Fxq 'lmcache_version=0.5.2+glm52dcp.4' <<<"${output_r29}"
grep -Fxq 'xgrammar_ref=v0.2.5' <<<"${output_r29}"
grep -Fxq 'xgrammar_transformers5_compat=1' <<<"${output_r29}"
grep -Fxq \
...
grep -Fxq 'composition=reproduce-r26' <<<"${output_r26}"
grep -Fxq \
  'image=voipmonitor/vllm:gilded-gnosis-v20-vllmf5981f1-sibbbdccc-fi801d57a-cu132-20260803-r26' \
  <<<"${output_r26}"
grep -Fxq \
  'version=0.11.2.dev280+gilded.gnosis.v20.vllmf5981f1.sibbbdccc.fi801d57a.cu132.20260803.r26' \
  <<<"${output_r26}"
grep -Fxq \
...
grep -Fxq \
  ...
  <<<"${output_r26}"
grep -Fxq \
  'lmcache_patch=releases/gilded-gnosis-v20-r26/lmcache/integration.patch' \
  <<<"${output_r26}"
grep -Fxq 'xgrammar_ref=v0.2.5' <<<"${output_r26}"
grep -Fxq \
  'instanttensor_repo=https://github.com/voipmonitor/InstantTensor.git' \
  <<<"${output_r26}"
grep -Fxq \
...
grep -Fxq \
  ...
  <<<"${output_r20}"
grep -Fxq \
  'launcher_commit=76b1cf0350709910208e61285adc955e34655136' \
  <<<"${output_r20}"
grep -Fxq \
  'lmcache_tree=a5aa59cc8edca462a3f4c198d17fd2b9c1a7ffaa' \
  <<<"${output_r20}"
grep -Fxq \
  'lmcache_patch=releases/gilded-gnosis-v20-r20/lmcache/integration.patch' \
  <<<"${output_r20}"
